...
async function cleanupJobArtifacts(jobId: string) {
  ...
  // 네가 쓰는 경로들(필요한 것만)
  const paths = [
    `renders/${jobId}/preview.png`,
    `renders/${jobId}/preview.jpg`,
    `renders/${jobId}/preview.webp`,
    `renders/${jobId}/preview.svg`,
    `renders/${jobId}/meta.json`,
    `renders/${jobId}/bom.json`,
  ];

  await supabaseAdmin.storage.from(publicBucket).remove(paths);
}
/**
 * ✅ OpenAI: 분석 (JSON 응답)
 */
...
export async function POST(req: Request) {
  ...
  try {
    ...
    try {
      ...
      throw e; // 아래 outer catch에서 500/메시지 처리 or 여기서 502로 처리
    }
...
async function generateImageWithOpenAI2(inputImageUrl: string, prompt: string): Promise<string> {
  ...
  const model = process.env.OPENAI_IMAGE_EDIT_MODEL?.trim() || "gpt-image-1.5";
  const baseUrl = process.env.OPENAI_BASE_URL?.trim() || "https://api.openai.com";
  const project = process.env.OPENAI_PROJECT_ID?.trim();
  ...
  const body = {
    model,
    images: [{ image_url: inputImageUrl }], // ✅ 핵심
    prompt,
    size: "1024x1024", // ✅ gpt-image-1.5 edits size enum에 맞춤
    // response_format: "b64_json", // (문서에선 기본이 b64_json로 내려옴. 필요하면 유지)
  };
